...
from utils3D.augmentations import tensor_cutout, random_zoom, adjust_brightness, adjust_contrast, add_random_noise, random_flip_with_bbox
...
class LoadNiftisAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment:           
            ...
            img = adjust_contrast(img, p=self.hyp['prob_zoom'])
            img = adjust_brightness(img, p=self.hyp['prob_zoom'])
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                ...
                assert (l[:, 1:7] <= 1).all(), f'non-normalized or out of bounds coordinates {l[:, 1:][l[:, 1:] > 1]}'
